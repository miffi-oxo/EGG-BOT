...
def _build_raid_options(raid_data, default_value: str | None = None) -> list[discord.SelectOption]:
    ...
    options.append(
        discord.SelectOption(
            label=f"{OTHER_RAID_LABEL} (레이드 외 컨텐츠 모집)",
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if target_dt <= datetime.now(KST):
            retry_view = _RescheduleRetryView(
                ...
                {"date": date_str, "hour": hour, "minute": minute, "description": description_text},
            )
            await interaction.response.send_message(
        ...
        await self.cog.update_raid_schedule_only(interaction, self.raid_id, date_str, hour, minute, description_text)
...
class TitleEditModal(discord.ui.Modal):
    def __init__(self, cog: "RaidScheduleCog", raid_id: str):
        super().__init__(title="제목 수정")
        self.cog = cog
        self.raid_id = raid_id
        entry = cog.raids.get(raid_id) or {}
        self.title_input = discord.ui.TextInput(
            label="새 제목", default=entry.get("title", ""), max_length=100
        )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message("✅ 제목이 수정됐어요.", ephemeral=True)
...
class CharacterNameModal(discord.ui.Modal):
    def __init__(
        ...
        raid_id: str,
        *,
        mode: str,  # "apply" | "rename" | "force" | "force_rename"
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if self.mode in ("force", "force_rename"):
            ...
            if self.mode in ("apply", "force") and min_level and info["level"] < min_level:
...
class ManagePanelView(discord.ui.View):
    ...
    @discord.ui.button(label="제목수정", style=discord.ButtonStyle.blurple)
